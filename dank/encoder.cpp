...
std::string DFAEncoder::unrank( const InfInt c_in ) {
    std::string retval;
    ...
    InfInt words_in_slice = getNumWordsInLanguage( _fixed_slice, _fixed_slice );
    ...
    for (i=1; i<=_fixed_slice; i++) {
        if (_delta_dense.at(q)) {
            ...
            char_cursor = char_index.toUnsignedInt();
        } else {
...
InfInt DFAEncoder::rank( const std::string X ) {
    //mpz_class retval = 0;
    InfInt retval = 0;
...
InfInt DFAEncoder::getNumWordsInLanguage( const uint32_t min_word_length,
