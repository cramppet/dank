CC := g++
CFLAGS := -Wall -Wextra -shared -std=c++14 -O3 -fPIC -I dank/_dank/include/

PYBIND11_INCLUDES := $(shell python3 -m pybind11 --includes)
PYBIND11_LIBS := $(shell python3-config --ldflags)
PYBIND11_SUFFIX := $(shell python3-config --extension-suffix)

SOURCES := $(wildcard dank/_dank/*.cpp)
OBJECTS := $(patsubst dank/_dank/%.cpp, obj/%.o, ${SOURCES})
